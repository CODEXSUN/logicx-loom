...
const privateKeyPath = join(directory, "github_logicx-loom");
const publicKeyPath = `${privateKeyPath}.pub`;
const comment = options.comment ?? `logicx-loom-server@${hostname()}`;
...
function printHelp() {
  output.log(`Generate a temporary ED25519 keypair for LOGICX_LOOM server access to GitHub.

Usage:
  npm run github:ssh-key
  npm run github:ssh-key -- --comment "logicx-loom-server"
