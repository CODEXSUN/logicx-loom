...
const tag = `v-${version}`;
const apkName = `LogicX Loom-${version}-release.apk`;
...
function prepareReleaseFiles() {
  ...
  const manifest = {
    apkUrl: `https://github.com/CODEXSUN/logicx-loom/releases/download/${tag}/${apkName}`,
    mandatory,
...
function publishRelease() {
  ...
  run("git", ["tag", "-a", tag, "-m", `LogicX Loom mobile ${tag}`]);
