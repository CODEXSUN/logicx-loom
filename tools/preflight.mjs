...
const child = spawn(
  ...
  [...config.args, ...(app.endsWith("-web") ? ["--host", host, "--port", String(port)] : [])],
  {
    ...
    env: {
      ...
      ...(app === "platform-api"
        ? {
            LOGICX_LOOM_DB_FRESH_SESSION_FILE: join(
              tmpdir(),
              `logicx-loom-platform-fresh-${process.pid}.done`
            )
          }
        : {}),
...
async function freePort(port, host) {
  ...
  if (process.env.LOGICX_LOOM_DEV_PORT_POLICY === "abort") {
    console.error(
      "  x Port policy is abort. Stop the existing process or change LOGICX_LOOM_DEV_PORT_POLICY.\n"
    );
    process.exit(1);
  }
