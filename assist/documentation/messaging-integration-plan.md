...
LogicX Loom is a standalone, single-client application: one Fastify API
...
- No WebSocket infrastructure exists. `@fastify/websocket` (and `ws`) are not
  installed. Phase 1 adds them to `@logicx-loom/platform-api` only.
...
- `@fastify/websocket` and `ws` (added to `@logicx-loom/platform-api`).
...
| --- | --- | --- |
| 1 | conversations, members, messages, WebSocket auth, connection manager, persistence, basic send/receive, sync.request | `app.ts`, `schema.ts`, `logicx-loom-database.ts`, boundary/lifecycle check tools, `platform-api` deps |
