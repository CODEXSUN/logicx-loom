...
Loom Data owns machine JSON ingestion and local event persistence. `POST /loomdata` accepts machine
payloads. The authenticated Dashboard reads the stored event stream.
...
iShop owns its LogicX Loom routes and UI contracts. LogicX iShop and ERPNext catalog records remain
...
supplies presentation primitives. LogicX Loom does not import parent-repository source or compose
