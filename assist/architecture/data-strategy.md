...
LogicX Loom uses one MariaDB database selected only by `DB_NAME`.
...
Loom Data stores each machine JSON payload without changing its structure. The first Dashboard
version shows raw payloads. Later versions can project the stored data into organized tables.

## CRM

Frappe is the source of truth for CRM enquiries and related records. LogicX Loom must not create,
...
LogicX Loom must not create, seed, mirror, synchronize, or cache Estimate business tables locally.
List, reference, create, read, and update requests use the signed-in user's verified Frappe identity.

## Quotation

Frappe is the source of truth for Quotation records. Each LogicX Loom quotation is linked to its
...
Item Group, Brand, variant, and image references. LogicX Loom does not mirror or cache iShop data.

## Safety

- Database names and endpoints come from `.env`.
- Encrypted integration credentials require `LOGICX_LOOM_INTEGRATION_ENCRYPTION_KEY`.
- Destructive reset requires `LOGICX_LOOM_DB_RESET_CONFIRM=DROP_DATABASE`.
- Production reset additionally requires `LOGICX_LOOM_ALLOW_PRODUCTION_DB_RESET=1`.
