...
const vijay: MessagingActor = { email: "vijay@logicxloom.in", id: 3, name: "Vijay", uuid: "vvvv" };
...
test("local message media is stored below its conversation root", async () => {
  const root = await mkdtemp(join(tmpdir(), "logicx-loom-media-"));
  try {
    ...
    assert.equal((await storage.read(42, stored.key))?.toString(), "Hello LogicX Loom");
