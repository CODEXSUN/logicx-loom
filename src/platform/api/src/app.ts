...
import {
  bootstrapLogicXLoomDatabase,
  closeLogicXLoomDatabase,
  getLogicXLoomDatabase
} from "./database/logicx-loom-database.js";
...
import { loomDataModule } from "./modules/loom-data/index.js";
...
export async function createApp() {
  console.info("[logicx-loom.boot] bootstrap started");
  await bootstrapLogicXLoomDatabase();

  const app = await createApiApp({
    appName: "LogicX Loom API",
    cookieSecret: env.JWT_SECRET,
    corsOrigins: platformWebOrigins(),
    environment: env.NODE_ENV,
    shutdownHooks: [closeLogicXLoomDatabase],
    tenantContext: false
  });
  const healthChecks: HealthCheck[] = [
    {
      name: "logicx-loom-api",
      check: () => ({
        details: {
  ...
  await registerModules(
    modules,
    {
      ...
      notificationPublisher: createNotificationPublisher(getLogicXLoomDatabase())
    },
    {
  ...
  console.info("[logicx-loom.boot] bootstrap completed");
