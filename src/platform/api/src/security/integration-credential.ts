...
function credentialKey(namespace: string) {
  const operatorKey = env.LOGICX_LOOM_INTEGRATION_ENCRYPTION_KEY.trim() || env.JWT_SECRET;
  return createHash("sha256").update(`logicx-loom:${namespace}:${operatorKey}`).digest();
}
