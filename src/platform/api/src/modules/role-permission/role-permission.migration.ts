...
export async function migrateRolePermissionModule(database: Kysely<LogicXLoomDatabase>) {
  await sql
    .raw(
...
async function column(db: Kysely<LogicXLoomDatabase>, name: string) {
  const r = await sql<{
    count: number | string;
...
async function addFk(db: Kysely<LogicXLoomDatabase>, name: string, col: string, parent: string) {
  const r = await sql<{
    count: number | string;
