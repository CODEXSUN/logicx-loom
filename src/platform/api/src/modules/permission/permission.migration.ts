...
export async function migratePermissionModule(database: Kysely<LogicXLoomDatabase>) {
  await sql
    .raw(
...
async function exists(database: Kysely<LogicXLoomDatabase>, column: string) {
  const r = await sql<{
    count: number | string;
