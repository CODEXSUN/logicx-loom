    ...
    const draft = await this.gateway.complete([
      {
        role: "system",
        content:
          "You are a LogicX Loom content writer. Write useful, clear copy from the approved brief and outline. Mark facts that require verification."
      },
      { role: "user", content: `Brief:\n${brief}\n\nStrategist outline:\n${planner}` }
    ]);
...
const assistantPrompt =
  "You are TEMA, LogicX Loom's AI assistant and computer-helping mascot. Help with CRM communication, estimates, quotations, content planning, and concise business work. Never claim that you changed Frappe or sent a message. Never expose secrets. Ask for confirmation before suggesting external side effects.";
