...
function nearestEnvironmentFile() {
  const configuredPath = env.LOGICX_LOOM_ENV_FILE_PATH.trim();
  if (configuredPath) {
...
function saveErrorMessage(error: unknown) {
  ...
  if (code === "EACCES" || code === "EPERM" || code === "EROFS") {
    return "The LogicX Loom .env file is not writable by the API process. Make the configured runtime .env file writable, then save again.";
  }
  return "The Frappe settings could not be saved to the LogicX Loom .env file.";
}
