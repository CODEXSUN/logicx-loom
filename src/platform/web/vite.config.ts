...
export default defineConfig(({ command, mode }) => {
  ...
  return {
    ...
    ...(command === "serve"
      ? {
          server: {
            allowedHosts: ["logicx-loom.test", "localhost", "127.0.0.1"],
            fs: {
...
function logicXLoomMobileRelease() {
  const source = resolve(repositoryDir, "storage/mobile/release");
  const destination = resolve(repositoryDir, "dist/platform/web/storage/mobile/release");
  return {
    name: "logicx-loom-mobile-release",
    closeBundle() {
