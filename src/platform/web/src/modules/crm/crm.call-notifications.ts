...
export const crmInAppNotificationEvent = "logicx-loom:crm-in-app-notification";
const notificationPermissionEvent = "logicx-loom:browser-notification-permission";
const notificationPreferenceEvent = "logicx-loom:crm-call-notifications";
const notificationPreferenceStorageKey = "logicx-loom.crm.calls.notifications.enabled";
...
export function sendCrmTestNotification() {
  ...
  const notification = {
    ...
    id: `test-${Date.now()}`,
    title: "LogicX Loom notification test"
  };
...
function showNotification(value: CallNotification) {
  const browserNotification = new Notification(value.title, {
    body: value.body,
    tag: `logicx-loom-crm-${value.id}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
  });
