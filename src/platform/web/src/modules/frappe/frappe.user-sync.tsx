...
export function FrappeUserSyncWorkspace({ canImport }: { canImport: boolean }) {
  ...
  async function addUser(record: FrappeUserPreview) {
    try {
      ...
      if (imported.created) {
        ...
        toast.info("User is already in LogicX Loom", { description: imported.user.email });
      }
    } catch {}
  }
  ...
  return (
    <WorkspacePage
      ...
      description="Preview enabled Frappe System Users and add them to LogicX Loom."
...
function TemporaryPasswordDialog({
  ...
  return (
    <Dialog open={Boolean(result)} onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>User added to LogicX Loom</DialogTitle>
          <DialogDescription>
