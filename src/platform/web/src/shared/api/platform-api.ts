...
const SESSION_EXPIRED_WARNING_KEY = "logicx-loom_session_expired_warning";
  ...
  try {
    ...
    if (responseText) {
      try {
        ...
        throw new Error(`LogicX Loom API returned an invalid response (${response.status}).`);
      }
    }
    if (!envelope) throw new Error("LogicX Loom API returned an empty response.");
