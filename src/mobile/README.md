...
LogicX Loom Mobile is a fresh Ionic React and Capacitor application. It owns its mobile-first
screens, navigation, gestures, and Tailwind-based visual system. It consumes shared LogicX Loom
...
Set `VITE_MOBILE_API_URL` to the deployed LogicX Loom API origin and path. Production values must use
HTTPS. The API must allow the native origins through `PLATFORM_WEB_ORIGINS`:

```env
VITE_MOBILE_API_URL=https://app.logicxloom.in/api/platform
...
npm.cmd run assets:generate --workspace @logicx-loom/mobile
...
```powershell
$env:VITE_MOBILE_API_URL="https://app.logicxloom.in/api/platform"
