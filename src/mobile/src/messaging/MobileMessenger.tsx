...
export function MobileMessenger({ actorEmail, onThreadChange }: { actorEmail: string; onThreadChange: (value: boolean) => void }) {
  ...
  return <section className="logicx-loom-messenger"><header className="logicx-loom-messenger-header"><div><p className="logicx-loom-eyebrow">MESSAGES</p><h1>Chats</h1></div><button aria-label="New chat" className="logicx-loom-icon-button" onClick={() => setNewChatOpen(true)} type="button"><IonIcon icon={addOutline} /></button></header><label className="logicx-loom-chat-search"><IonIcon icon={searchOutline} /><IonInput onIonInput={(event) => setContactQuery(event.detail.value ?? "")} placeholder="Search conversations" value={contactQuery} /></label>{error ? <p className="logicx-loom-error">{error}</p> : null}<div className="logicx-loom-chat-list">{loading ? <p className="logicx-loom-empty">Loading chats…</p> : null}{!loading && !conversations.length ? <p className="logicx-loom-empty">No chats yet. Start one with a LogicX Loom contact.</p> : null}{!loading && conversations.length && !filteredConversations.length ? <p className="logicx-loom-empty">No matching chats.</p> : null}{filteredConversations.map((conversation) => <button className="logicx-loom-chat-row" key={conversation.id} onClick={() => { setActive(conversation); onThreadChange(true); }} type="button"><Avatar name={conversationTitle(conversation, actorEmail)} /><span className="logicx-loom-chat-copy"><strong>{conversationTitle(conversation, actorEmail)}</strong><small>{conversation.lastMessage?.content || "Start a conversation"}</small></span><span className="logicx-loom-chat-meta"><time>{formatTime(conversation.updatedAt)}</time>{conversation.unreadCount ? <b>{conversation.unreadCount}</b> : null}</span></button>)}</div><ContactPicker contacts={contacts} onClose={() => setNewChatOpen(false)} onCreateGroup={startGroup} onSearch={setContactQuery} onSelect={start} open={newChatOpen} /></section>;
}
...
function Thread({ actorEmail, conversation, error, messages, onBack, onError, onMessagesChange, onSend }: { actorEmail: string; conversation: Conversation; error: string; messages: Message[]; onBack: () => void; onError: (message: string) => void; onMessagesChange: (update: (messages: Message[]) => Message[]) => void; onSend: (content: string, type: MessageType, metadata: Record<string, unknown>, replyToMessageId?: number) => Promise<boolean> }) {
  ...
  return <section className="logicx-loom-thread"><header className="logicx-loom-thread-header"><button aria-label="Back to chats" className="logicx-loom-icon-button" onClick={onBack} type="button"><IonIcon icon={arrowBackOutline} /></button><Avatar name={title} /><div className="logicx-loom-thread-identity"><strong>{title}</strong><small>Online</small></div><div className="logicx-loom-thread-actions"><button aria-expanded={Boolean(messageSearch)} aria-label="Search messages" className="logicx-loom-toolbar-button" onClick={() => setMessageSearch((value) => value ? "" : " ")} type="button"><IonIcon icon={searchOutline} /></button><button aria-expanded={optionsOpen} aria-label="Conversation options" className="logicx-loom-toolbar-button" onClick={() => setOptionsOpen((open) => !open)} type="button"><IonIcon icon={ellipsisVerticalOutline} /></button></div>{optionsOpen ? <nav className="logicx-loom-thread-menu"><button type="button">Contact info</button><button type="button">Shared media</button><button type="button">Mute notifications</button></nav> : null}</header>{messageSearch ? <label className="logicx-loom-thread-search"><IonIcon icon={searchOutline} /><IonInput autofocus onIonInput={(event) => setMessageSearch(event.detail.value ?? "")} placeholder="Search messages" value={messageSearch.trim()} /></label> : null}{error ? <p className="logicx-loom-error logicx-loom-thread-error">{error}</p> : null}<div className="logicx-loom-message-scroll">{messages.length && !visibleMessages.length ? <p className="logicx-loom-thread-empty">No matching messages.</p> : null}{visibleMessages.map((message) => <MessageBubble key={message.id} message={message} mine={message.senderEmail === actorEmail} onLongPress={setActionMessage} pinned={pinned.includes(message.id)} reaction={[...new Set(message.reactions.map((item) => item.emoji))].join(" ") || undefined} selected={selected === message.id} starred={starred.includes(message.id)} />)}{!messages.length ? <p className="logicx-loom-thread-empty">No messages yet. Send the first one.</p> : null}</div><MessageComposer onCancelReply={() => setReply(undefined)} onSend={onSend} reply={reply} /><IonActionSheet buttons={actions} isOpen={Boolean(actionMessage)} onDidDismiss={() => setActionMessage(undefined)} /></section>;
}
...
function MessageComposer({ onCancelReply, onSend, reply }: { onCancelReply: () => void; onSend: (content: string, type: MessageType, metadata: Record<string, unknown>, replyToMessageId?: number) => Promise<boolean>; reply?: Message | undefined }) {
  ...
  return <footer className="logicx-loom-composer">{attachmentMenu ? <div className="logicx-loom-attachment-menu"><button onClick={() => imageRef.current?.click()} type="button"><IonIcon icon={imageOutline} />Image</button><button onClick={() => videoRef.current?.click()} type="button"><IonIcon icon={videocamOutline} />Video</button><button onClick={() => fileRef.current?.click()} type="button"><IonIcon icon={documentOutline} />File</button></div> : null}{mention !== null && mentionContacts.length ? <div className="logicx-loom-mention-menu">{mentionContacts.map((contact) => <button key={contact.id} onClick={() => setDraft((value) => insertMention(value, contact.name))} type="button"><Avatar name={contact.name} /><span><strong>{contact.name}</strong><small>{contact.email}</small></span></button>)}</div> : null}{reply ? <div className="logicx-loom-reply-preview"><span>Replying to {reply.senderName}</span><button aria-label="Cancel reply" onClick={onCancelReply} type="button"><IonIcon icon={closeOutline} /></button></div> : null}{attachment ? <div className="logicx-loom-attachment-preview"><IonIcon icon={attachment.kind === "IMAGE" ? imageOutline : attachment.kind === "VIDEO" ? videocamOutline : documentOutline} /><span>{attachment.name}</span><button aria-label="Remove attachment" onClick={() => setAttachment(undefined)} type="button"><IonIcon icon={closeOutline} /></button></div> : null}{emojiOpen ? <div className="logicx-loom-emoji-strip">{[...reactions, "😀", "🎉", "✅"].map((emoji) => <button key={emoji} onClick={() => { setDraft((value) => value + emoji); setEmojiOpen(false); }} type="button">{emoji}</button>)}</div> : null}<input accept="*/*" className="logicx-loom-file-input" onChange={(event) => void pick(event.target.files?.[0])} ref={fileRef} type="file" /><input accept="image/*" className="logicx-loom-file-input" onChange={(event) => void pick(event.target.files?.[0])} ref={imageRef} type="file" /><input accept="video/*" className="logicx-loom-file-input" onChange={(event) => void pick(event.target.files?.[0])} ref={videoRef} type="file" /><div className="logicx-loom-composer-shell"><button aria-label="Add attachment" className="logicx-loom-composer-icon" onClick={() => setAttachmentMenu((open) => !open)} type="button"><IonIcon icon={addOutline} /></button><button aria-label="Emoji" className="logicx-loom-composer-icon" onClick={() => setEmojiOpen((open) => !open)} type="button"><IonIcon icon={happyOutline} /></button><textarea onChange={(event) => { setDraft(event.target.value); event.currentTarget.style.height = "auto"; event.currentTarget.style.height = `${Math.min(event.currentTarget.scrollHeight, 128)}px`; }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit(); } }} placeholder="Type a message · @mention · /task" rows={1} value={draft} />{draft.trim() || attachment ? null : <button aria-label="Voice message" className="logicx-loom-composer-icon" type="button"><IonIcon icon={micOutline} /></button>}</div>{draft.trim() || attachment ? <button aria-label="Send" className="logicx-loom-send" onClick={() => void submit()} type="button"><IonIcon icon={paperPlaneOutline} /></button> : null}{error ? <p className="logicx-loom-composer-error">{error}</p> : null}</footer>;
}
...
function MessageBubble({ message, mine, onLongPress, pinned, reaction, selected, starred }: { message: Message; mine: boolean; onLongPress: (message: Message) => void; pinned: boolean; reaction?: string | undefined; selected: boolean; starred: boolean }) {
  ...
  return <article className={`logicx-loom-message ${mine ? "mine" : ""} ${selected ? "is-selected" : ""}`} onContextMenu={(event) => { event.preventDefault(); clear(); onLongPress(message); }} onTouchCancel={clear} onTouchEnd={clear} onTouchMove={clear} onTouchStart={hold}>{pinned || starred ? <span className="logicx-loom-message-flags">{pinned ? <IonIcon icon={pinOutline} /> : null}{starred ? <IonIcon icon={starOutline} /> : null}</span> : null}{message.replyToMessageId ? <span className="logicx-loom-reply-label">Reply</span> : null}{attachment ? <AttachmentView attachment={attachment} /> : null}{deliveryLabel(message) ? <span className="logicx-loom-delivery-label">Sent to {deliveryLabel(message)}</span> : null}<p>{message.content}</p><span className={`logicx-loom-message-meta ${message.status === "READ" ? "is-read" : ""}`}><time>{formatTime(message.createdAt)}</time>{mine ? <IonIcon icon={message.status === "READ" ? checkmarkDoneOutline : message.status === "DELIVERED" ? checkmarkDoneOutline : checkmarkOutline} /> : null}</span>{reaction ? <span className="logicx-loom-reaction">{reaction}</span> : null}</article>;
}
...
function ContactPicker({ contacts, onClose, onCreateGroup, onSearch, onSelect, open }: { contacts: MessagingContact[]; onClose: () => void; onCreateGroup: (title: string, members: MessagingContact[]) => void; onSearch: (value: string) => void; onSelect: (contact: MessagingContact) => void; open: boolean }) {
  ...
  return <IonModal breakpoints={[0, .72]} initialBreakpoint={.72} isOpen={open} onDidDismiss={close}><section className="logicx-loom-contact-picker"><header><div><p className="logicx-loom-eyebrow">{groupMode ? "NEW GROUP" : "NEW CHAT"}</p><h2>{groupMode ? "Add people" : "Choose a contact"}</h2></div><div><button aria-label={groupMode ? "Start a direct chat" : "Create group"} className="logicx-loom-icon-button" onClick={() => setGroupMode((value) => !value)} type="button"><IonIcon icon={groupMode ? arrowBackOutline : addOutline} /></button><button aria-label="Close" className="logicx-loom-icon-button" onClick={close} type="button"><IonIcon icon={closeOutline} /></button></div></header>{groupMode ? <IonInput className="logicx-loom-group-title" fill="outline" onIonInput={(event) => setGroupTitle(event.detail.value ?? "")} placeholder="Group name" value={groupTitle} /> : null}{selected.length ? <p className="logicx-loom-group-selection">{selected.map((contact) => contact.name).join(", ")}</p> : null}<label className="logicx-loom-chat-search"><IonIcon icon={searchOutline} /><IonInput autofocus onIonInput={(event) => onSearch(event.detail.value ?? "")} placeholder="Search people" /></label>{!contacts.length ? <p className="logicx-loom-empty">No active contacts are available. Ask an administrator to add a user.</p> : null}{contacts.map((contact) => <button aria-pressed={groupMode && selected.some((item) => item.id === contact.id)} className="logicx-loom-contact-row" key={contact.id} onClick={() => groupMode ? toggle(contact) : onSelect(contact)} type="button"><Avatar name={contact.name} /><span><strong>{contact.name}</strong><small>{contact.email}</small></span>{groupMode && selected.some((item) => item.id === contact.id) ? <IonIcon className="logicx-loom-contact-check" icon={checkmarkCircleOutline} /> : null}</button>)}{groupMode ? <button className="logicx-loom-create-group" disabled={!groupTitle.trim() || selected.length < 2} onClick={() => onCreateGroup(groupTitle.trim(), selected)} type="button">Create group</button> : null}</section></IonModal>;
}
function Avatar({ name }: { name: string }) { return <span className="logicx-loom-avatar">{name.split(/\s+/u).map((part) => part[0]).slice(0, 2).join("").toUpperCase()}</span>; }
function AttachmentView({ attachment }: { attachment: Attachment }) {
  const [open, setOpen] = useState(false);
  const source = useAttachmentSource(attachment.dataUrl);
  if (!source) return <span className="logicx-loom-message-file"><IonIcon icon={documentOutline} />Loading {attachment.name}</span>;
  if (attachment.kind === "IMAGE") return <img alt={attachment.name} className="logicx-loom-message-image" src={source} />;
  if (attachment.kind === "VIDEO") return <video className="logicx-loom-message-video" controls src={source} />;
  if (!isPdf(attachment)) return <button className="logicx-loom-message-file" onClick={() => void downloadMessagingAttachment(attachment.dataUrl, attachment.name)} type="button"><IonIcon icon={documentOutline} />{attachment.name}</button>;
  return <>
    <section aria-label={`${attachment.name} document`} className="logicx-loom-pdf-card">
      <button aria-label={`View ${attachment.name}`} className="logicx-loom-pdf-preview" onClick={() => setOpen(true)} type="button">
        <span className="logicx-loom-pdf-badge">PDF</span>
        <span className="logicx-loom-pdf-copy"><strong>{attachment.name}</strong><small>{formatAttachmentMeta(attachment)}</small></span>
      </button>
      <footer>
    ...
    <IonModal className="logicx-loom-pdf-modal" isOpen={open} onDidDismiss={() => setOpen(false)}>
      <section className="logicx-loom-pdf-viewer">
        <header><div><strong>{attachment.name}</strong><small>{formatAttachmentMeta(attachment)}</small></div><button aria-label="Close document" className="logicx-loom-icon-button" onClick={() => setOpen(false)} type="button"><IonIcon icon={closeOutline} /></button></header>
