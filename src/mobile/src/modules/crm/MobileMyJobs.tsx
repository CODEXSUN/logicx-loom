...
export function MobileMyJobs() {
  ...
  return (
    <section className="logicx-loom-jobs-page bg-background text-foreground">
      <div className="logicx-loom-page-heading"><h1>My Jobs</h1></div>
      <div className="logicx-loom-job-summary" aria-label="My job summary">
      ...
      {!loading && !error ? (
        <div className="logicx-loom-job-list">
...
function MobileJobDetail({ onBack, onRecordChange, record }: { onBack: () => void; onRecordChange: (record: CrmEnquiry) => void; record: CrmEnquiry }) {
  ...
  return (
    <section className="logicx-loom-job-detail">
      <header className="logicx-loom-job-detail-header">
        <button aria-label="Back to My Jobs" className="logicx-loom-icon-button" onClick={onBack} type="button"><IonIcon icon={arrowBackOutline} /></button>
        <div><strong>Job details</strong><small>Live Frappe enquiry</small></div>
      </header>
      <JobCard record={record} />
      <section className="logicx-loom-job-timeline" aria-label="Job activity">
        <h2>Activity</h2>
        ...
        )) : <p className="logicx-loom-subtitle">No activity has been recorded yet.</p>}
      </section>
      <footer className="logicx-loom-job-footer">
        ...
        <button className="logicx-loom-job-add" onClick={() => setActionOpen((value) => !value)} type="button">{actionOpen ? "Close actions" : "Add activity"}</button>
      </footer>
    </section>
  );
}
...
function JobCard({ onOpen, record }: { onOpen?: () => void; record: CrmEnquiry }) {
  ...
  return <article className={`logicx-loom-job-card is-${record.priority} bg-card text-card-foreground border-border shadow-sm`} onClick={onOpen}>
    <div className="logicx-loom-job-card-top"><i aria-label={`${record.priority} priority`} role="img" /><span>#{record.id}</span><em>·</em><small>{record.enquiryGroup || "Enquiries"}</small><strong>{record.assignedTo?.name ?? "Unassigned"}</strong><button aria-label="Job actions" className="logicx-loom-icon-button" onClick={(event) => { event.stopPropagation(); setMenuOpen((value) => !value); }} type="button"><IonIcon icon={ellipsisVertical} /></button></div>
    <p className="logicx-loom-job-customer">{record.customerName || record.customer || "Customer enquiry"}</p>
    <h2>{plainText(record.title) || "Untitled enquiry"}</h2>
    <p className="logicx-loom-job-enquiry-detail">{detail}</p>
    <div className="logicx-loom-job-card-bottom"><time>{relativeTime(record.updatedAt)} · Due {formatDueDate(record.schedules[0]?.scheduledOn)}</time><span className={`logicx-loom-job-status is-${record.status}`}><IonIcon icon={statusIcon(record.status)} />{record.status.replaceAll("-", " ")}</span></div>
    {menuOpen ? <nav className="logicx-loom-job-menu" onClick={(event) => event.stopPropagation()}><a href={phone ? `tel:${phone}` : undefined}><IonIcon icon={callOutline} />Call</a><a href={phone ? `https://wa.me/91${phone}` : undefined} rel="noreferrer" target="_blank"><IonIcon icon={logoWhatsapp} />WhatsApp</a><button onClick={() => navigator.geolocation?.getCurrentPosition(() => undefined)} type="button"><IonIcon icon={locationOutline} />Location</button></nav> : null}
  </article>;
}
...
function State({ label, tone }: { label: string; tone?: "error" }) { return <div className={`logicx-loom-job-state ${tone === "error" ? "is-error" : ""}`}>{label.includes("Loading") ? <IonSpinner name="crescent" /> : null}<span>{label}</span></div>; }
