...
export function MobileCallCapture() {
  ...
  return <section className="logicx-loom-page logicx-loom-call-capture">
    <p className="logicx-loom-eyebrow">CALLS</p><h1>Call history</h1>
    <p className="logicx-loom-subtitle">Select an attended call, then record the customer request as a CRM enquiry.</p>
    <IonButton className="logicx-loom-call-import" disabled={loading} onClick={() => void loadHistory()}><IonIcon icon={downloadOutline} slot="start" />{loading ? <IonSpinner name="crescent" /> : "Refresh call history"}</IonButton>
    {calls.length ? <section className="logicx-loom-call-history" aria-label="Attended call history">{calls.map((call) => <button className={selected?.id === call.id ? "is-selected" : ""} key={call.id} onClick={() => { setSelected(call); setSaved(""); }} type="button"><IonIcon icon={callOutline} /><span><strong>{call.direction === "incoming" ? "Incoming call" : "Outgoing call"}</strong><small>{formatCall(call)}</small></span></button>)}</section> : null}
    {selected ? <section className="logicx-loom-call-card">
    ...
    {saved ? <p className="logicx-loom-call-success">{saved}</p> : null}{error ? <p className="logicx-loom-error">{error}</p> : null}
  </section>;
}
