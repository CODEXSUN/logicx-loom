...
CHECK_ONLY=false
ALLOW_DIRTY=false
LOCK_FILE="${TMPDIR:-/tmp}/logicx-loom-update.lock"
...
Safely update an existing LogicX Loom Docker installation while preserving:
...
if [[ "$CHECK_ONLY" != true ]]; then
  command -v flock >/dev/null 2>&1 || {
    echo "flock is required to serialize LogicX Loom deployment updates." >&2
    exit 69
  }
  exec 9>"$LOCK_FILE"
  flock -n 9 || {
    echo "Another LogicX Loom update is already running (lock: $LOCK_FILE)." >&2
    exit 75
  }
fi
...
validate_release_contract() {
  ...
  for key in LOGICX_LOOM_VERSION LOGICX_LOOM_IMAGE_TAG LOGICX_LOOM_MIGRATION_COMPATIBLE_VERSION; do
...
inspect_source_state() {
  ...
  git -C "$ROOT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
    echo "LogicX Loom update source is not a Git worktree: $ROOT_DIR" >&2
    exit 78
  }
  source_commit="$(git -C "$ROOT_DIR" rev-parse HEAD)"
  if [[ -n "$(git -C "$ROOT_DIR" status --porcelain --untracked-files=normal)" ]]; then
    source_dirty=true
    echo "WARNING: the LogicX Loom Git worktree contains uncommitted or untracked files." >&2
...
for key in \
  ...
  LOGICX_LOOM_NETWORK; do
  require_setting "$DEPLOY_ENV" "$key"
done

project="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_COMPOSE_PROJECT logicx-loom)"
api_container="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_API_CONTAINER_NAME logicx-loom-api)"
web_container="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_WEB_CONTAINER_NAME logicx-loom-web)"
network="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_NETWORK logicx-loom-network)"
network_external="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_NETWORK_EXTERNAL false)"
image_registry="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_IMAGE_REGISTRY logicx-loom)"
image_tag="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_IMAGE_TAG local)"
backup_retention="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_BACKUP_RETENTION 10)"
minimum_backup_mb="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_UPDATE_MIN_BACKUP_FREE_MB 1024)"
minimum_docker_mb="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_UPDATE_MIN_DOCKER_FREE_MB 5120)"
...
for setting in \
  "LOGICX_LOOM_BACKUP_RETENTION:$backup_retention" \
  "LOGICX_LOOM_UPDATE_MIN_BACKUP_FREE_MB:$minimum_backup_mb" \
  "LOGICX_LOOM_UPDATE_MIN_DOCKER_FREE_MB:$minimum_docker_mb"; do
  key="${setting%%:*}"
  value="${setting#*:}"
  ...
  cat >"${metadata_file}.partial" <<EOF
{
  ...
  "migrationCompatibilityVersion": "$(file_value "$DEPLOY_ENV" LOGICX_LOOM_MIGRATION_COMPATIBLE_VERSION)",
...
prune_old_backups() {
  ...
  while IFS= read -r old_backup; do
    ...
    if [[ "$count" -gt "$retention" ]]; then
      ...
      rm -f -- "$resolved_backup_dir/logicx-loom-deployment-$backup_timestamp.json"
...
for service_spec in "$api_container:api" "$web_container:web"; do
  ...
  docker container inspect "$container" >/dev/null 2>&1 || {
    echo "Existing LogicX Loom $service container was not found: $container" >&2
...
if [[ "$network_external" == true ]]; then
  mariadb_container="$(
    file_value "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb
  )"
  ...
  mariadb_container="$(
    file_value "$DEPLOY_ENV" LOGICX_LOOM_MARIADB_CONTAINER_NAME logicx-loom-mariadb
  )"
  safe_docker_name "$mariadb_container"
  docker container inspect "$mariadb_container" >/dev/null 2>&1 || {
    echo "Existing LogicX Loom MariaDB container was not found: $mariadb_container" >&2
    exit 69
  }
...
if container_is_running "$api_container"; then
  docker exec "$api_container" node -e \
    "require('node:fs').accessSync(process.env.LOGICX_LOOM_ENV_FILE_PATH, require('node:fs').constants.R_OK | require('node:fs').constants.W_OK)" \
    >/dev/null 2>&1 || {
...
echo
echo "LogicX Loom Docker update plan"
...
if [[ "$CHECK_ONLY" == true ]]; then
  echo
  echo "Existing LogicX Loom Docker deployment is ready to update."
  exit 0
fi

if [[ "$ASSUME_YES" != true ]]; then
  read -r -p "Build and update the existing LogicX Loom containers? [Y/n] " confirmation
...
metadata_file="$resolved_backup_dir/logicx-loom-deployment-$timestamp.json"
...
api_url="http://${probe_address}:$(file_value "$DEPLOY_ENV" LOGICX_LOOM_API_HOST_PORT 19350)/health"
web_url="http://${probe_address}:$(file_value "$DEPLOY_ENV" LOGICX_LOOM_WEB_HOST_PORT 19360)/health"
...
echo
echo "LogicX Loom Docker update completed."
