...
RUNTIME_ENV="${LOGICX_LOOM_RUNTIME_ENV:-$ROOT_DIR/.env}"
RUNTIME_ENV_EXAMPLE="$ROOT_DIR/.env.example"
DEPLOY_ENV="${LOGICX_LOOM_DEPLOY_ENV:-$CONTAINER_DIR/deploy.env}"
...
Interactive standalone LogicX Loom container installation.
...
Configuration:
  .env                    LogicX Loom runtime, application, and Frappe settings
...
Included:
  ...
  - Repository-owned UI
  - LogicX Loom Platform API and Web
  - Reused existing Docker network and MariaDB, or LogicX Loom-owned infrastructure

Excluded:
  - CXApp, Billing, DevKit, Mail, TMApp, Trades, Redis, Media, and all other stacks

The setup builds only this self-contained LogicX Loom repository. It asks whether
to reuse an existing Docker network and running MariaDB container or create a
dedicated LogicX Loom network and MariaDB. LogicX Loom uses only MariaDB; optional
Redis and Media containers are detected and left untouched.
If dedicated LogicX Loom MariaDB data already exists, setup separately asks
whether to reuse or freshly recreate only the LogicX Loom database volume.
EOF
}
...
prepare_deploy_environment() {
  ...
  case "$(file_value "$DEPLOY_ENV" DB_USER)" in
    ""|root)
      set_file_value "$DEPLOY_ENV" DB_USER logicx_loom
      echo "Configured the dedicated deployment database user: logicx-loom"
      ;;
  esac
  set_default_if_empty "$DEPLOY_ENV" DB_NAME logicx_loom_db
  ...
  set_file_value "$DEPLOY_ENV" LOGICX_LOOM_VERSION "$version"
  set_file_value "$DEPLOY_ENV" LOGICX_LOOM_IMAGE_TAG "$version"
  set_file_value "$DEPLOY_ENV" LOGICX_LOOM_MIGRATION_COMPATIBLE_VERSION "$version"
  set_default_if_empty "$DEPLOY_ENV" LOGICX_LOOM_BACKUP_RETENTION 10
  set_default_if_empty "$DEPLOY_ENV" LOGICX_LOOM_UPDATE_MIN_BACKUP_FREE_MB 1024
  set_default_if_empty "$DEPLOY_ENV" LOGICX_LOOM_UPDATE_MIN_DOCKER_FREE_MB 5120
...
configure_deploy_environment() {
  ...
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_COMPOSE_PROJECT "Compose project" logicx-loom
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_IMAGE_REGISTRY "Image registry/prefix" logicx-loom
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_API_CONTAINER_NAME "API container name" logicx-loom-api
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_WEB_CONTAINER_NAME "Web container name" logicx-loom-web
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_BIND_ADDRESS "Host bind address" 127.0.0.1
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_API_HOST_PORT "API host port" 19350
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_WEBSOCKET_HOST_PORT "WebSocket host port" 19351
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_WEB_HOST_PORT "Web host port" 19360
  prompt_setting "$DEPLOY_ENV" DB_NAME "Application database name" logicx_loom_db
  prompt_setting "$DEPLOY_ENV" DB_USER "Application database user" logicx-loom
...
prepare_runtime_environment() {
  ...
  if [[ "$infrastructure_mode" == shared ]]; then
    database_host="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb)"
  else
  ...
  set_file_value "$RUNTIME_ENV" PLATFORM_API_PORT \
    "$(file_value "$DEPLOY_ENV" LOGICX_LOOM_API_INTERNAL_PORT 9350)"
  set_file_value "$RUNTIME_ENV" PLATFORM_WEB_PORT \
    "$(file_value "$DEPLOY_ENV" LOGICX_LOOM_WEB_HOST_PORT 19360)"
  set_file_value "$RUNTIME_ENV" LOGICX_LOOM_DB_FRESH_ON_START 0
  set_file_value "$RUNTIME_ENV" LOGICX_LOOM_DB_RESET_CONFIRM ""
  set_file_value "$RUNTIME_ENV" LOGICX_LOOM_ALLOW_PRODUCTION_DB_RESET 0
...
configure_runtime_environment() {
  ...
  prompt_setting "$RUNTIME_ENV" INITIAL_ADMIN_EMAIL \
    "Initial administrator email" admin@logicxloom.in
...
select_infrastructure_mode() {
  ...
  while true; do
    read -r -p \
      "Reuse an existing Docker network and MariaDB, or create dedicated LogicX Loom infrastructure? [reuse/dedicated] " \
      answer
...
prepare_shared_infrastructure() {
  ...
  mariadb="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb)"
  network="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_NETWORK logicx-loom-network)"
  redis="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_SHARED_REDIS_CONTAINER_NAME cxapp-redis)"
  media="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MEDIA_CONTAINER_NAME cxapp-media)"
  ...
  for name in "$redis" "$media"; do
    if container_is_running "$name"; then
      echo "Detected shared container (left untouched; LogicX Loom does not consume it): $name"
    else
      echo "Shared optional container is unavailable and is not required by LogicX Loom: $name"
    fi
  done

  root_password="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MARIADB_ROOT_PASSWORD)"
  [[ -n "$root_password" ]] || {
    echo "Shared MariaDB root password is unavailable." >&2
    echo "Configure LOGICX_LOOM_SHARED_MARIADB_ROOT_PASSWORD in $DEPLOY_ENV." >&2
    exit 78
  }
}
...
configure_shared_infrastructure() {
  ...
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MARIADB_CONTAINER_NAME \
    "Existing MariaDB container name" cxapp-mariadb
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_NETWORK "Existing Docker network" logicx-loom-network
  set_file_value "$DEPLOY_ENV" LOGICX_LOOM_NETWORK_EXTERNAL true
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_SHARED_REDIS_CONTAINER_NAME \
    "Shared Redis container name (detected only)" cxapp-redis
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MEDIA_CONTAINER_NAME \
    "Shared Media container name (detected only)" cxapp-media
  prompt_secret "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MARIADB_ROOT_PASSWORD \
...
configure_dedicated_infrastructure() {
  ...
  echo
  echo "Dedicated LogicX Loom infrastructure settings"
  if [[ "$(file_value "$DEPLOY_ENV" LOGICX_LOOM_NETWORK_EXTERNAL false)" == true ]]; then
    set_file_value "$DEPLOY_ENV" LOGICX_LOOM_NETWORK logicx-loom-network
  fi
  set_file_value "$DEPLOY_ENV" LOGICX_LOOM_NETWORK_EXTERNAL false
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_MARIADB_CONTAINER_NAME \
    "Dedicated MariaDB container name" logicx-loom-mariadb
  container="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_MARIADB_CONTAINER_NAME logicx-loom-mariadb)"
  while docker container inspect "$container" >/dev/null 2>&1 &&
    ! container_is_compose_service "$container" \
      "$(file_value "$DEPLOY_ENV" LOGICX_LOOM_COMPOSE_PROJECT logicx-loom)" mariadb; do
    ...
    set_file_value "$DEPLOY_ENV" LOGICX_LOOM_MARIADB_CONTAINER_NAME "$replacement"
    container="$replacement"
  done
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_NETWORK "Dedicated Docker network" logicx-loom-network
  prompt_setting "$DEPLOY_ENV" LOGICX_LOOM_MARIADB_DATA_VOLUME \
    "MariaDB data volume" logicx-loom-mariadb-data
...
compose() {
  LOGICX_LOOM_RUNTIME_ENV_FILE="$RUNTIME_ENV" docker compose \
...
ensure_available_host_port() {
  ...
  bind_address="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_BIND_ADDRESS 127.0.0.1)"
  while true; do
...
validate_deploy_environment() {
  local key
  for key in \
    LOGICX_LOOM_COMPOSE_PROJECT \
    LOGICX_LOOM_API_CONTAINER_NAME \
    LOGICX_LOOM_WEB_CONTAINER_NAME \
    LOGICX_LOOM_MARIADB_CONTAINER_NAME \
    LOGICX_LOOM_NETWORK \
    LOGICX_LOOM_MARIADB_DATA_VOLUME; do
    safe_docker_name "$(file_value "$DEPLOY_ENV" "$key")"
  done
  ensure_available_host_port \
    LOGICX_LOOM_API_HOST_PORT \
    "API host port" \
    "$(file_value "$DEPLOY_ENV" LOGICX_LOOM_API_CONTAINER_NAME logicx-loom-api)"
  ensure_available_host_port \
    LOGICX_LOOM_WEBSOCKET_HOST_PORT \
    "WebSocket host port" \
    "$(file_value "$DEPLOY_ENV" LOGICX_LOOM_API_CONTAINER_NAME logicx-loom-api)"
  ensure_available_host_port \
    LOGICX_LOOM_WEB_HOST_PORT \
    "Web host port" \
    "$(file_value "$DEPLOY_ENV" LOGICX_LOOM_WEB_CONTAINER_NAME logicx-loom-web)"
}
...
select_database_mode() {
  ...
  while true; do
    read -r -p "Reuse or freshly recreate LogicX Loom MariaDB data? [reuse/fresh] " answer
    case "${answer:-reuse}" in
      reuse|Reuse|REUSE)
        ...
        ;;
      fresh|Fresh|FRESH)
        read -r -p "Type FRESH $volume to delete only this LogicX Loom volume: " confirmation
...
reconcile_database_user() {
  ...
  if [[ "$infrastructure_mode" == shared ]]; then
    container="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MARIADB_CONTAINER_NAME cxapp-mariadb)"
    root_password="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_SHARED_MARIADB_ROOT_PASSWORD)"
  else
    container="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_MARIADB_CONTAINER_NAME logicx-loom-mariadb)"
...
SQL
  then
    echo "Could not reconcile the LogicX Loom MariaDB application user." >&2
...
if [[ "$infrastructure_mode" == dedicated ]]; then
  database_volume="$(file_value "$DEPLOY_ENV" LOGICX_LOOM_MARIADB_DATA_VOLUME logicx-loom-mariadb-data)"
  safe_docker_name "$database_volume"
  database_mode="$(select_database_mode "$database_volume")"
  infrastructure_label="dedicated LogicX Loom network and MariaDB"
else
  database_volume=""
...
echo
echo "Standalone LogicX Loom deployment plan"
echo "  Runtime env: $RUNTIME_ENV"
echo "  Deploy env: $DEPLOY_ENV"
echo "  Source: self-contained LogicX Loom monorepo"
echo "  Infrastructure: $infrastructure_label"
echo "  MariaDB data: $database_mode"
echo "  Images: internal Framework/UI -> LogicX Loom Platform API/Web"
echo "  Runtime: LogicX Loom API, LogicX Loom Web, selected MariaDB"
echo "  Excluded: CXApp, TMApp, Billing, DevKit, Mail, Trades, Redis, and Media"
read -r -p "Build and apply this standalone LogicX Loom installation? [Y/n] " confirmation
...
echo
echo "LogicX Loom standalone installation completed."
echo "Web: http://$(file_value "$DEPLOY_ENV" LOGICX_LOOM_BIND_ADDRESS 127.0.0.1):$(file_value "$DEPLOY_ENV" LOGICX_LOOM_WEB_HOST_PORT 19360)/"
echo "API health: http://$(file_value "$DEPLOY_ENV" LOGICX_LOOM_BIND_ADDRESS 127.0.0.1):$(file_value "$DEPLOY_ENV" LOGICX_LOOM_API_HOST_PORT 19350)/health"
echo "WebSocket: ws://$(file_value "$DEPLOY_ENV" LOGICX_LOOM_BIND_ADDRESS 127.0.0.1):$(file_value "$DEPLOY_ENV" LOGICX_LOOM_WEBSOCKET_HOST_PORT 19351)/ws/messaging"
