...
import '../../core/api/messaging_realtime_client.dart';
...
class MessagesSamplePage extends StatefulWidget {
  const MessagesSamplePage({
    required this.api,
  ...
  @override
  State<MessagesSamplePage> createState() => _MessagesSamplePageState();
}

class _MessagesSamplePageState extends State<MessagesSamplePage> {
...
class _ConversationThreadState extends State<_ConversationThread> {
  ...
  late final MessagingRealtimeClient _realtime;
  ...
  void initState() {
    ...
    _realtimeSubscription = _realtime.messages.listen((event) {
      if (event.conversationId == widget.conversation.id) _refresh();
    });
    _realtime.connect();
    _realtime.subscribe(widget.conversation.id);
  }
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: Theme.of(context).textTheme.labelSmall
                  ?.copyWith(color: mine ? const Color(0xFFDDE6FF) : null),
            ),
          ],
        ),
      ),
